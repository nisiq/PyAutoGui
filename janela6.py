...
class Aplicacao():
    ...
    def botoes(self):
        ...
        self.btDimensao = Button(self.janela, text="Dimensao", command=self.dimension)
        self.btDimensao.place(relx=0.05, rely=0.50, relwidth=0.1, relheight=0.10)

        self.btPosicao = Button(self.janela, text="Posicão", command=self.position)
        self.btPosicao.place(relx=0.20, rely=0.50, relwidth=0.1, relheight=0.10)

        self.btMover = Button(self.janela, text="Mover", command=self.move)
        self.btMover.place(relx=0.35, rely=0.50, relwidth=0.1, relheight=0.10)

        self.btAlerta = Button(self.janela, text="Alerta", command=self.alert)
    ...
    def alert(self):
        print(pyautogui.confirm(text='Prometo fazer os exercícios', title='Feriado', buttons=['OK', 'Cancel']))
