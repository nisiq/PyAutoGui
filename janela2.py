...
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg #Permite gráfico dentro da Janela do Tkinter
...
class Aplicacao():
    ...
    def labels(self):
        ...
        self.lbStatus = Label(self.janela, text='', fg='red', bg='#f0f')
        self.lbStatus.place(relx=0.05, rely=0.21, relwidth=0.90, relheight=0.02)

        self.lbGrafico = Label(self.frame_3, text='', fg='red', bg='#b0bfa9')
        self.lbGrafico.place(relx=0.1, rely=0.01, relwidth=0.9, relheight=0.98)
    ...
    def grafic(self):
        ...
        canva = FigureCanvasTkAgg(figura, self.lbGrafico)
        canva.get_tk_widget().grid(row=0, column=0)

        #fruits = ['apple', 'blueberry', 'cherry', 'orange']
        fruits = ['apple', 'blueberry', 'cherry', 'orange', 'Lindomar']
        counts = [40, 100, 30, 55, 200]
        bar_labels = ['red', 'blue', '_red', 'orange', 'cor']
